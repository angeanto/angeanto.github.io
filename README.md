...
The homepage hero in `index.md` places the Accelerator enrollment and scorecard card above a compact LinkedIn card. Both use `.hero-promo-card` styles and the shared gold primary button. `_data/accelerator.yml` holds the product URL, November 2026 enrollment cohort, price, Stripe Payment Link and MailerLite embed ID. All enrollment buttons, including the scorecard results, read `checkout_url` from this shared file. When replacing the Stripe Payment Link, update that value; update cohort and price fields only when the offer also changes. Enrollment sections on the product page explicitly identify the November cohort; the course has eight weekly sessions of approximately 1.5 hours, with participants voting on session dates. The original owner-supplied architecture image is retained under `assets/images/lab/accelerator-architecture.png`. No payment credentials belong in this repository.
...
`assets/images/branding/data-conscious-wordmark-transparent.png` is the owner-supplied “DataConscious Labs Logo-text.png”, cropped to its visible bounds (935×232) while preserving its existing alpha transparency and original colors. `_config.yml` selects it for the shared masthead. `_includes/masthead.html` displays the wordmark above “by Antonis Angelakis” in a single home link; the brand name is provided by the image with accessible alt text. Responsive sizes are in `assets/css/main.scss`.

The owner-supplied “DataConscious Labs Logo-no-text.png” supplies the 16, 32 and 96px PNG favicons, the multi-resolution ICO (16/32/48px), and the 180px Apple touch icon in the same directory, all named `favicon-transparent*`. `_includes/head/custom.html` declares them across pages. Icons are cropped, centered on a transparent 640×640 canvas with padding, then resized; artwork, alpha transparency and colors are preserved. Both source PNGs already contain transparency; no background removal or artwork regeneration is needed. The previous assets remain available at their original URLs. Use new filenames or a version suffix when replacing icons to avoid stale browser caches. No new dependencies or ignore rules are needed; builds stay outside the repository.
